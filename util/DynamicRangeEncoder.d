module util.DynamicRangeEncoder;
...
class dynamicranger {
	...
	public:
	this(Params params, uint[] _counts) {
		counts = _counts.dup;
	...
	void newfile(char[] _filename) {
		filename = _filename;
	}
	
	void newcounts(uint[] _counts) {
		counts = _counts.dup;
	}
	...
	uint getfilelen() {
		...
		uint temp = filebuf.length;
		filebuf.close();
		
		return temp;
	}
	
	BitArray encode(out uint len) {
		bool found;
		double[] percents;
		double[] cdfs;
		ubyte[] curkeys;
		...
		//=calculate percents================================
		total = 0;
		for( uint s = 0; s < num_symbols; s++) {
			total += counts[s];
		}
		
		double cdf = 0.0;
		for( uint s = 0; s < num_symbols; s++) {
			//Stdout.formatln("{}:  {}", s, counts[s]);
			percents[s] = cast(double)counts[s]/cast(double)total;
			cdfs[s] = cdf;
			cdf += percents[s];
			/*if (percents[s] > 0){
				Stdout.formatln("cdf:  {}; percent:  {}", cdfs[s], percents[s]);
			}*/
		}
		//---------------------------------------------------
	...
	void decode(BitArray bitcode, uint len) {
		double[] percents;
		double[] cdfs;
		uint low = 0, high = (1 << (32-1));
		//XXX: new codes
		...
		for( uint s = 0; s < num_symbols; s++) {
			...
			/*if (percents[s] > 0){
				Stdout.formatln("{}:  cdf:  {}; percent:  {}", s, cdfs[s], percents[s]);
			}*/
		}
		//---------------------------------------------------
